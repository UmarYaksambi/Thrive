...
export function JoinClassroom({ userId, existingMemberships }: JoinClassroomProps) {
  const [inviteCode, setInviteCode] = useState('');
  const [joining, setJoining] = useState(false);
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  const [memberships, setMemberships] = useState(existingMemberships);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-500">Join classrooms and access course materials</p>
      </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900">
                        {membership.classroom?.name || 'Unknown Classroom'}
                      </h3>
