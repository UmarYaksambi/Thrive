...
import { addDownload, getDownload, deleteDownload, getAllDownloads, updateProgress } from '@/lib/db';
...
export function useDownload() {
  ...
  // Download a resource
  const downloadResource = useCallback(async (resource: any) => {
    const { id, title, creator, type, resource_url } = resource;
    ...
    try {
      ...
      while (true) {
        ...
        if (total > 0) {
          ...
          setProgress((prev) => new Map(prev).set(id, percent));
          await updateProgress(id, {
            id,
            progress: percent,
            status: 'downloading',
  ...
  const getOfflineUrl = useCallback(async (id: string) => {
    ...
    const typedBlob = new Blob([blob], { type: correctType });
