...
export default function ChatPage() {
  ...
  const [isCreatingMastery, setIsCreatingMastery] =
  ...
  const sendMessage = async (
    text: string,
    options: { hidden?: boolean; topic?: string } = {}
  ) => {
    ...
    try {
      ...
      while (!done) {
        ...
        // Parse special tags
        const confirmMatch = accumulatedText.match(
          /\[CONFIRM_MASTERY:\s*(.*?)\]/
        );
        const successMatch = accumulatedText.match(
          /\[LESSON_COMPLETE:\s*(.*?):\s*(.*?)\]/
        );
  ...
  const handleResumeMastery = (topic: Topic) => {
    ...
    const nextSubtopic = topic.subtopics.find(
      (s) => !s.completed
    );
  ...
  const handleDeleteTopic = async (
    ...
    if (
      !confirm(
        'Delete this mastery track? This cannot be undone.'
      )
    )
      return;

    try {
      await fetch(`/api/chat/mastery/delete?id=${id}`, {
        method: 'DELETE',
      });
      await fetchMastery();
      ...
      if (
        deletedTopic &&
        activeTopic === deletedTopic.topic_name
      ) {
        setActiveTopic(null);
      }
    } catch (e) {
  ...
  const handleConfirmMastery = async (
    topicName: string,
    messageId: string
  ) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await sendMessage(
          `Yes, let's start learning ${topicName}!`,
          {
            hidden: false,
  ...
  const handleDenyMastery = async (messageId: string) => {
    ...
    await sendMessage(
      'No thanks, just explain it normally.'
    );
  };
                ...
                {isLoadingHistory ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p className="text-gray-400 text-lg">
                      Start a conversation or select a topic
                      to learn
                    </p>
                  </div>
                ) : (
                  messages.map((message) => (
                    ...
                    >
                      ...
                      {message.is_confirmation &&
                        message.confirmation_topic && (
