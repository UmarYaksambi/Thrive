 ...
 */
async function deleteUserStorageFiles(supabase: any, userId: string) {
  ...
  if (files && files.length > 0) {
    ...
    const filesToRemove = files.map((x: any) => `${userId}/${x.name}`);
...
export async function uploadAvatar(formData: FormData) {
  ...
  if (!user || !file) throw new Error("Unauthorized or missing file")
  
  if (file.size > MAX_FILE_SIZE) throw new Error("File size exceeds 2MB limit.")
