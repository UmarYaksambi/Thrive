...
import { Globe, User as UserIcon, LogOut, Camera, Loader2, Trash2 } from 'lucide-react';
import { toast } from "sonner";
import { cn } from '@/lib/utils';
import { updateProfile, deleteAccountAction, uploadAvatar, removeAvatar } from './actions';
import { logout } from '@/app/login/actions';

export function SettingsForm({ profile, email }: { profile: any; email: string | undefined }) {
    const [learningLevel, setLearningLevel] = useState(profile?.learning_level || 'Beginner');
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 2 * 1024 * 1024) {
            toast.error("File too large", { 
                description: "The selected file is larger than 2MB. Please choose a smaller image." 
            });
    ...
    return (
        <form action={handleSave} className="max-w-2xl mx-auto space-y-6 pb-20">
            ...
            <div className="bg-white rounded-3xl p-8 shadow-sm">
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <p className="text-xs text-gray-400">JPG, PNG, GIF or WebP. Max size of 2MB</p>
                            </div>
                        </div>
                    </div>

                    <div>
                        <label className="block text-sm font-semibold text-[#151313] mb-2">Full Name</label>
                        <input
                            name="fullName"
                            type="text"
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-[#151313] mb-2">Email</label>
                        <input
                            type="email"
                            disabled
                            value={email || ''}
            ...
            <div className="bg-white rounded-3xl p-8 shadow-sm">
                ...
                <div className="space-y-6">
                    <div>
                        <label className="block text-sm font-semibold text-[#151313] mb-3">Language</label>
                        <select name="language" defaultValue={profile?.preferred_language || 'English'} className="w-full px-6 py-3 rounded-full border-2 border-gray-200 focus:border-[#fccc42] font-semibold bg-white outline-none">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-[#151313] mb-3">Learning Level</label>
                        <input type="hidden" name="learningLevel" value={learningLevel} />
                            ...
                            {['Beginner', 'Intermediate', 'Advanced'].map((level) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        learningLevel === level ? 'bg-[#fccc42] text-[#151313]' : 'bg-gray-100 text-[#151313] hover:bg-gray-200'
                                    )}
                                >
                                    {level}
                                </button>
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-[#151313] mb-3">Interests</label>
                        <div className="flex flex-wrap gap-3">
                            {['Web Dev', 'Design', 'Python', 'Leadership', 'Data Science'].map((interest) => (
                                <div key={interest}>
                                    <input type="checkbox" name="interests" value={interest} checked={selectedInterests.includes(interest)} className="hidden" readOnly />
                                    <button
                                        type="button"
                                        onClick={() => toggleInterest(interest)}
                                        className={cn(
                                            "px-4 py-2 rounded-full font-semibold transition-colors",
                                            selectedInterests.includes(interest) ? "bg-[#fccc42] text-[#151313]" : "bg-gray-100 text-[#151313] hover:bg-[#fccc42]/20"
                                        )}
                                    >
                                        {interest}
                                    </button>
            ...
            <div className="flex gap-4 justify-center pt-4">
                <button 
                    ...
                    className={cn(
                        ...
                        isSaving ? "opacity-70 cursor-not-allowed" : "hover:bg-[#f4b91a]"
                    )}
                >
