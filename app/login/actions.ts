 ...
 * FIXED: Marked as async to handle Next.js 15 cookies() promise.
 */
      ...
      cookies: {
        ...
        set(
          ...
          try {
            ...
            // Handle Server Component read-only restriction
          }
        },
...
export async function login(formData: FormData) {
  // FIXED: Await the client initialization
...
export async function signup(formData: FormData) {
  ...
  if (data.user) {
    ...
    if (serviceRoleKey) {
      // Note: This client doesn't use request cookies, so simple mocks are fine here
